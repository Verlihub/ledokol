...
for k, v in pairs (table_mode) do
	if string.find (tolow (nick), k) or string.find (ip, k) then
		...
		elseif v == "num" then
			ret = string.gsub (ret, "[oOîÎ]", "0")
			ret = string.gsub (ret, "[iIlL]", "1")
			ret = string.gsub (ret, "[eEåÅ]", "3")
			ret = string.gsub (ret, "[aAàÀ÷×]", "4")
			ret = string.gsub (ret, "[sS]", "5")
			ret = string.gsub (ret, "[áÁøØ]", "6")
			ret = string.gsub (ret, "[tTòÒ%?]", "7")
			ret = string.gsub (ret, "[yYóÓ]", "9")
		end
...
conf = conf.."\r\n"
	...
	conf = conf .. "\r\n [::] chatfloodonecount = " .. table_sets ["chatfloodonecount"]
	conf = conf .. "\r\n [::] chatfloodoneint = " .. table_sets ["chatfloodoneint"]
